...
the full non-PDF browser regression against the current executable baseline of **2300/2300 PASS
...
dense telemetry cascade, while two mechanism-pilot assertions verify the horizontal dependency
rail, Answer Key interruption state and Accessible vertical rail in normal and grayscale.
...
| `C1C2-VIS01` | Lunar Greenhouse; S T3 p1, A T3 p2, AK T3 p1 | Conventional six-box flow; A uses a separate plain vertical list. | 2 · Causal mechanism/pathway | Reproductive telemetry rail: anther/pollen → physical agitation → stigma → pollen tube → fertilization → fruit, with numbered nodes and failure/interruption status; deterministic HTML/CSS/SVG. | Preserve six exact bank entries and fields; horizontal S/AK and vertical A remain one grammar. | Pores/structures downstream may remain capable; failed Step 2 interrupts rather than destroys them. | P1–P6; exact-bank parity; AK failure-state check | `IMPLEMENTED-CANDIDATE · 14/14 PILOT STATIC PASS · 2300/2300 BROWSER GATE PENDING` |
...
- full browser harness: current executable baseline **2300/2300 PASS, 0 JavaScript errors**;
- `git diff --check`.
