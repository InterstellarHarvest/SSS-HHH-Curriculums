...
Accessible vertical response sizing is a central authoring primitive, not case CSS. Future SSS and HHH packages classify every Accessible response through the shared `layout-overrides.schema.v1.json` contract: substantial response fields are explicitly eligible, and every protected response is explicitly locked with a reason. An unclassified response is a validation failure. The editor supplies the same vertical-only handles, 4px snapping, bounds, page safety, drafts, sparse persistence, and export/print behavior. CER and all Student, Teacher, and Answer Key fields remain protected from this primitive.
