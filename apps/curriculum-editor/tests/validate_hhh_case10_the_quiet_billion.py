...
OWNER_APPROVAL_RECORD = HISTORY / "CASE10_OWNER_APPROVAL_v0.1.md"
RELEASE_RECORD_PATH = "hhh/campaign-2/case-10-the-quiet-billion/history/release-v0.1.json"
APPROVAL_DATE = "2026-08-22"
UNIT_PATH = "hhh/campaign-2/case-10-the-quiet-billion"
...
OWNER_PRINTABLE_BASELINE = "990dce582f12d2f726b45b7c039ef0d34adc5c29"
...
OWNER_VISUAL_STATEMENT = "CASE10_OWNER_SCREEN_REVIEW_PASS"
OWNER_PRINT_STATEMENT = "CASE10_OWNER_PRINT_PASS"
...
# The Case 10 review line, recorded verbatim so a later edit cannot quietly invent a
# disposition that never happened or drop one that did.
REVIEW_LINE_COMMITS = ("17f81c1cfe7e19a496af428a90c2793d7986d77b",
                       "456ee72a5b0cce4784199a976a794fd215f256f8",
                       OWNER_PRINTABLE_BASELINE)
INDEPENDENT_DISPOSITIONS = ("CASE10_INDEPENDENT_REVIEW_FAIL",
                            "CASE10_INDEPENDENT_REMEDIATION_VERIFICATION_PASS")
DISPLAY_ORDER = 12
...
def main() -> int:  # noqa: C901 - one flat assertion sequence, deliberately readable
    ...
    # --- IDENTITY AND RELEASED LIFECYCLE ------------------------------------
    ...
    # Release conversion turned the candidate-state lifecycle assertions below into
    ...
    results.check("history/ holds only the owner approval record and the release record",
                  ...
                  == ["CASE10_OWNER_APPROVAL_v0.1.md", "release-v0.1.json"],
    ...
    results.check("the shared release-history schema exists for the direct validation below",
                  RELEASE_SCHEMA.is_file(), str(RELEASE_SCHEMA))
    results.check("exactly one Case 10 release record exists in the tree",
                  [q.name for q in UNIT.rglob("release-*.json")] == ["release-v0.1.json"],
                  [str(q.relative_to(UNIT)) for q in UNIT.rglob("release-*.json")])
    ...
    for key, filename in SOURCE_FILES:
        try:
            blobs[key] = hashlib.sha256(subprocess.run(
                ["git", "show", f"{certified}:{UNIT_PATH}/source/{filename}"],
    ...
    results.check("the README preserves the printable baseline separately from the certified commit",
                  OWNER_PRINTABLE_BASELINE in readme_text)
    ...
    for filename in FROZEN_PRINTABLE_SOURCES:
        try:
            baseline_blobs[filename] = hashlib.sha256(subprocess.run(
                ["git", "show", f"{OWNER_PRINTABLE_BASELINE}:{UNIT_PATH}/source/{filename}"],
                cwd=ROOT, check=True, capture_output=True).stdout).hexdigest()
        except (subprocess.CalledProcessError, FileNotFoundError):
    ...
    # behind an identical leaf count cannot pass.
    try:
        baseline_registry = json.loads(re.sub(
            r"^\s*window\.[A-Z0-9_]+\s*=\s*", "",
            subprocess.run(["git", "show",
                            f"{OWNER_PRINTABLE_BASELINE}:{UNIT_PATH}/source/task-registry.js"],
    ...
    results.check("the task registry top-level key order is unchanged from the owner-approved baseline",
                  baseline_registry is not None and list(baseline_registry) == list(registry),
                  json.dumps({"baseline": list(baseline_registry or {})[:8],
                              "released": list(registry)[:8]}))
    ...
    results.check("no record merges the two owner statements into one quotation",
                  all(f"{OWNER_VISUAL_STATEMENT} and {OWNER_PRINT_STATEMENT}" not in text
                      and f"{OWNER_VISUAL_STATEMENT}, {OWNER_PRINT_STATEMENT}" not in text
                      for text in (approval_text, readme_text, json.dumps(release))))
    results.check("the README records both owner gates with both exact statements",
                  OWNER_VISUAL_STATEMENT in readme_text and OWNER_PRINT_STATEMENT in readme_text)
    results.check("the owner approval record states both owner gates and the approval date",
                  ...
                  APPROVAL_DATE)
    # --- THE REVIEW LINE, PRESERVED TRUTHFULLY -------------------------------
    for commit in REVIEW_LINE_COMMITS:
        results.check(f"the owner approval record records the review-line commit {commit[:7]}",
                      commit in approval_text, commit)
    for disposition in INDEPENDENT_DISPOSITIONS:
        results.check(f"the release record and the owner approval record record {disposition} exactly",
                      disposition in json.dumps(release) and disposition in approval_text,
                      disposition)
    results.check("no independent disposition is invented beyond the two that occurred",
                  not re.search(r"CASE10_INDEPENDENT(?!_REVIEW_FAIL|_REMEDIATION_VERIFICATION_PASS)",
                                json.dumps(release) + approval_text + readme_text),
                  sorted(set(re.findall(r"CASE10_INDEPENDENT[A-Z_]*",
                                        json.dumps(release) + approval_text + readme_text))))
    results.check("no post-owner independent review is claimed for this case",
                  "POST_OWNER" not in (json.dumps(release) + approval_text + readme_text).upper()
                  or "none is claimed" in (json.dumps(release) + approval_text).lower())
    results.check("the release record names the exactly two independent-review blockers",
                  "adoption" in json.dumps(release).lower()
                  and "12.26" in json.dumps(release) and "20.09" in json.dumps(release)
                  and "nearly doubled" in json.dumps(release))
    ...
    results.check("the owner approval record states that no canonical PDF was approved or required",
                  "No canonical PDF was produced, approved, or required" in approval_text)
    ...
    # unvalidated against its own schema. This closes that gap for Case 10, reusing the
    ...
    history_schema = json.loads(RELEASE_SCHEMA.read_text(encoding="utf-8"))
    ...
    RELEASE_METADATA_TOKENS = ("originalReleaseApprovalCommit", "canonicalSourceApprovalCommit",
                               ...
                               "acceptedPrintStatus", "APPROVED_STABLE", "OWNER_REVIEW_PASS",
                               OWNER_PRINTABLE_BASELINE, certified,
